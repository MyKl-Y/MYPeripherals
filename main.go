...
import (
	...
	"github.com/MyKl-Y/MYPeripherals/db"
)
...
type Metrics struct {
	Keypresses      int
	KeypressDetails map[uint16]int //map[string]int
	MouseClicks     int
...
func initLogger() {
	...
	case "darwin":
		// macOS
		logDir = filepath.Join(homeDir, ".config", "MYPeripherals")
	case "windows":
		// Windows
		logDir = "C:\\ProgramData\\MYPeripherals\\Logs\\"
	case "linux":
		// Linux
		logDir = filepath.Join(homeDir, ".config", "MYPeripherals")
	default:
	...
	logFile := filepath.Join(logDir, "MYPeripherals.log")
	...
	logger.Println("MYPeripherals started")
}
...
func loadConfig() (DBConfig, error) {
	...
	viper.AddConfigPath(".")
	viper.AddConfigPath("$HOME/.config/MYPeripherals")
...
func onReady() {
	...
	systray.SetTooltip("MYPeripherals")
	...
	go func() {
		for {
			...
			case <-mQuit.ClickedCh:
				logger.Println("MYPeripherals shutting down")
				systray.Quit()
				return
			}
		}
...
func collectMetrics() {
	metrics.KeypressDetails = make(map[uint16]int) //make(map[string]int)

	hook.Register(hook.KeyDown, nil, func(e hook.Event) {
		key := e.Rawcode //hook.RawcodetoKeychar(e.Rawcode)
		metrics.Keypresses++
		metrics.KeypressDetails[key]++

		// Track specific combinations (Command+C, Command+V, Command+X)
		/* if contains([]int{3675, 29, 3676}, int(e.Rawcode)) { // Command key
			switch hook.RawcodetoKeychar(key) {
				case "c":
					metrics.KeypressDetails["Command+C"]++
				case "v":
					metrics.KeypressDetails["Command+V"]++
				case "x":
					metrics.KeypressDetails["Command+X"]++
			}
		} */
...
func saveMetrics() {
	...
	if dbQueries == nil {
		...
		if err != nil {
			logger.Printf("failed to save overall metrics: %v", err)
			_ = tx.Rollback()
			return
		}

		// Save detailed keypress metrics
        for key, count := range metrics.KeypressDetails {
            _, err := tx.Exec(`
                INSERT INTO keypress_details (key, count)
                VALUES (?, ?)
            `, key, count)
            if err != nil {
                logger.Printf("Failed to save keypress details: %v", err)
                _ = tx.Rollback()
                return
            }
        }
	...
	// Save metrics to a PostgreSQL or another database via dbQueries
	...
	// Save detailed keypress metrics
    for key, count := range metrics.KeypressDetails {
        _, err := dbQueries.CreateKeypressDetail(context.Background(), db.CreateKeypressDetailParams{
            Key:   key,
            Count: int32(count),
        })
        if err != nil {
            logger.Printf("Error saving keypress details: %v", err)
        }
    }
...
func setupDefaultSQLite() error {
	...
	dbDir := filepath.Join(homeDir, ".config", "MYPeripherals")
	if err := os.MkdirAll(dbDir, 0755); err != nil {
		return fmt.Errorf("failed to create database directory: %w", err)
	}

	dbPath := filepath.Join(dbDir, "MYPeripherals.db")
	...
	_, err = sqlDb.Exec(`
		CREATE TABLE IF NOT EXISTS keypress_details (
